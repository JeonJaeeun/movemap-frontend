...
import { S } from "./ParentsProgramReviewWriteScreen.style";
...
import { postProgramReview } from "../../../api/review"; 

export default function ParentsProgramReviewWriteScreen() {
    const navigation = useNavigation();
    const route = useRoute();

    const selectedProgram = route.params?.program ?? null;
    ...
    return (
        <SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }}>
            <View style={S.Container}>
                ...
                <CustomText style={S.headerText}>프로그램 후기 작성</CustomText>

                {/* 프로그램 선택 */}
                <CustomText style={S.label}>프로그램을 선택해주세요!</CustomText>

                <TouchableOpacity
                    style={S.searchBox}
                    onPress={() => navigation.navigate("ParentsProgramSelect")}
                >
                    <SearchIcon width={20} height={20} />
                    <CustomText style={{ marginLeft: 10, color: "#000" }}>
                        {selectedProgram ? selectedProgram.name : "프로그램 선택"}
                    </CustomText>
                </TouchableOpacity>
